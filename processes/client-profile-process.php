...
//SAVE CLIENT PROFILE (Create or Update)
if(isset($_POST['save_profile'])){
    ...
    $client_profile_url = "Location: ../client/profile.php";
    ...
    if(empty($first_name) || empty($surname) || empty($phone_number) || empty($whatsapp_number) || 
       empty($bio) || empty($state) || empty($lga) || empty($address) || empty($id_type)){
        $_SESSION['error'] = "Please fill in all required fields.";
        header($client_profile_url);
        exit();
    }
    ...
    //Create instance of ClientProfile
    $clientProfileInstance = new ClientProfile();

    //Check if profile already exists
    $existing_profile = $clientProfileInstance->getClientProfileByAccountId($account_id, $pdo);
    ...
    if(isset($_FILES['profile_image']) && $_FILES['profile_image']['error'] !== UPLOAD_ERR_NO_FILE){
        ...
        $upload_result = $clientProfileInstance->uploadFile($_FILES['profile_image'], $upload_dir, $allowed_extensions, $max_size, 'profile_image');
    ...
    if(isset($_FILES['id_document']) && $_FILES['id_document']['error'] !== UPLOAD_ERR_NO_FILE){
        ...
        $upload_result = $clientProfileInstance->uploadFile($_FILES['id_document'], $upload_dir, $allowed_extensions, $max_size, 'id_');
    ...
    if($is_update){
        ...
        $result = $clientProfileInstance->editClientProfileByAccountId(
            $account_id, $first_name, $surname, $other_names, $phone_number, $whatsapp_number,
            $bio, $state, $lga, $address, $profile_image, $id_type, $id_document, $pdo
        );
        ...
        //Create new profile
        $result = $clientProfileInstance->createClientProfile(
            $account_id, $first_name, $surname, $other_names, $phone_number, $whatsapp_number,
            $bio, $state, $lga, $address, $profile_image, $id_type, $id_document, $pdo
        );
